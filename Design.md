...
- **Swipe-to-delete pada baris transaksi**: baris bisa digeser ke kiri untuk memunculkan tombol Hapus (background Signal Coral, ikon trash putih) tanpa perlu buka layar Detail dulu. Geser minimal ~35-40% lebar baris untuk "commit" ke posisi terbuka (snap ke posisi terbuka), kurang dari itu snap kembali ke posisi semula. Tap tombol Hapus yang muncul → konfirmasi singkat (opsional untuk aksi destruktif) → baris hilang dengan animasi collapse/fade, dan saldo transaksi-transaksi sesudahnya di-recalculate (SDD.md §4.3).
- **Transaksi baru "muncul" di list**: begitu transaksi baru disimpan dari bottom sheet, baris baru masuk ke posisi paling atas list dengan animasi slide-in dari atas + fade-in (durasi ~320ms, easing `easeOut`). Baris ini sekilas (~600-700ms) diberi highlight background hijau muda lembut sebelum bertransisi halus kembali ke background ledger normal (transparent) — memberi sinyal visual "ini baru ditambahkan" tanpa mengganggu alur baca list.
- **Transisi antar layar**: pakai slide horizontal standar (bukan fade/instan) untuk navigasi antar layar, terutama saat membuka layar Detail Transaksi dari Home — layar baru masuk dari kanan, layar sebelumnya sedikit bergeser ke kiri (efek push khas iOS/Android native), durasi ~250-300ms. Konsisten dipakai untuk semua navigasi push (Home → Detail, Home → History, Home → Export Preview), bukan cuma satu layar saja.
